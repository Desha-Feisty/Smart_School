...
async function test() {
    ...
    // Test Admin Registration (note: backend may not allow admin role)
    console.log('\n3. Register Admin (if allowed):');
    const admin = await makeRequest('/api/auth/register', 'POST', {
        name: 'Admin User',
        email: 'admin@test.com',
        password: 'password123',
        role: 'teacher' // trying teacher since admin might not be in enum
    });
    console.log(admin);
